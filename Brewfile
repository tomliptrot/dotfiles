...
brew "prek"                    # Pre-commit reimplemented in Rust
brew "railway"                 # Develop and deploy code with Railway
...
# Tapped brews
brew "anomalyco/tap/opencode"          # AI coding agent in the terminal
brew "hashicorp/tap/terraform"         # Tool to build, change, and version infrastructure
brew "hivemq/mqtt-cli/mqtt-cli"        # MQTT CLI for MQTT 5.0 and MQTT 3.1.1
brew "twilio/brew/twilio"              # Twilio CLI
...
cask "responsively"            # Browser tailored for responsive web app development
...
# Casks - Fonts
# Note: setup.sh also installs every `font-*-nerd-font` cask via `brew search`.
# These are the explicit picks; the rest get pulled in by that step.
cask "font-fira-code"
